...
这是 Autolabor C1 相机云台的 **ROS2 Humble 驱动与能力交付包**。当前冻结在 **Phase 1：RAW Hardware Acceptance**，目标不是承担导航或巡检任务编排，而是向任意任务控制系统提供一个稳定的：
...
## 当前主流程：Phase 1 RAW Hardware Acceptance

使用 `usb_cam` 原始图像流，优先验收 `3840x2160@30`。`camera_info_url` 为空，不启动 `image_proc`，不使用 `image_rect`，保留原始广角畸变；AcquireView 只消费 `/cv_camera0/image_raw`。相机门禁见 [CAMERA_OFFLINE_ACCEPTANCE.md](CAMERA_OFFLINE_ACCEPTANCE.md)。
...
请将设备路径替换为本机稳定的 `/dev/v4l/by-id/...` 和 `/dev/serial/by-id/...`。该门禁必须在真实 C1 已连接时执行；当前环境没有实机证据，因此不宣称通过。
...
- Phase 2：暂缓，保留 ROS2 `camera_calibration` 流程，详见 [CAMERA_CALIBRATION_ROS2.md](CAMERA_CALIBRATION_ROS2.md)。
- Phase 3：暂不实现 runtime 耦合；未来上层只依赖 [CAPABILITY_INTERFACE.md](CAPABILITY_INTERFACE.md) 中的 AcquireView capability。
